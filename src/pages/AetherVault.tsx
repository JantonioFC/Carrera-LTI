  ...
  return (
    <div className="h-full flex flex-col bg-navy-900 border-l border-navy-700/50" data-color-mode="dark">
      ...
      <div className="h-14 flex items-center justify-between px-4 border-b border-navy-700/50 bg-navy-950/80">
        ...
        <div className="flex items-center gap-3">
           <button onClick={handleCreateNote} className="flex items-center gap-1.5 px-3 py-1.5 bg-lti-blue hover:bg-lti-blue-dark text-white rounded-md text-sm font-medium transition-colors">
              <Plus size={16} />
              Nueva Nota
           </button>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Main Workspace */}
        <div className="flex-1 flex flex-col overflow-hidden relative" id="aether-graph-container">
          {viewMode === 'editor' && activeNote && (
            <div className="flex-1 flex bg-[#0d1117] overflow-hidden">
               ...
               <div className="w-64 border-l border-navy-700/50 bg-navy-950/20 p-4 overflow-y-auto hidden lg:block">
                  <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">Backlinks ({backlinks.length})</h3>
                  {backlinks.length > 0 ? (
                    <div className="space-y-3">
                      {backlinks.map(bl => (
                        <div key={bl.id} 
                          ...
                          <p className="text-sm font-medium text-lti-blue group-hover:underline">{bl.title}</p>
                          <p className="text-xs text-slate-400 mt-1 line-clamp-2">{bl.content.slice(0, 100)}...</p>
                        </div>
                      ))}
                    </div>
                  ) : (
                  ...
                  <div className="text-xs text-slate-400 space-y-2">
                    ...
                    <button 
                      onClick={() => {
                        if(confirm('¿Eliminar esta nota para siempre?')) deleteNote(activeNote.id);
                      }}
