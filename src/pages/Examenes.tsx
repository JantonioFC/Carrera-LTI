...
const Examenes: React.FC<ExamenesProps> = ({ calendarEvents, onUpdateCalendarEvents }) => {
  ...
  const handleAddExam = (e?: React.FormEvent) => {
    ...
    const updated = {
      ...
      [newExam.date]: [
        ...
        { title: `Examen: ${newExam.subject}`, topic: newExam.topic, type: "examen", time: "09:00" }
      ]
    };
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto space-y-6">
      <header className="flex items-center gap-3 mb-8 text-left">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Gestión de Exámenes</h1>
          <p className="text-slate-400 text-sm">Configura períodos y fechas de evaluación</p>
        </div>
      </header>
      ...
      <section className="card p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5 flex flex-col items-start">
            <label className="text-xs font-medium text-slate-400 uppercase">Inicio</label>
            <input 
              type="date" 
              value={academicDates.examStart}
              onChange={(e) => updateAcademicDates({ examStart: e.target.value })}
          ...
          <div className="space-y-1.5 flex flex-col items-start">
            <label className="text-xs font-medium text-slate-400 uppercase">Fin</label>
            <input 
              type="date" 
      ...
      <section className="space-y-3">
        <h2 className="text-slate-400 text-xs font-bold uppercase tracking-widest px-1">Próximos Exámenes</h2>
        <div className="space-y-2">
          {exams.length === 0 ? (
            ...
            exams.map((exam) => (
              <div key={exam.id} className="card p-4 flex items-center justify-between group hover:border-lti-orange/30 transition-all">
                <div className="flex items-center gap-4">
                  <div className="flex flex-col items-center justify-center p-2 bg-navy-800 rounded-lg min-w-[60px] border border-navy-700">
                    <span className="text-[10px] uppercase font-bold text-lti-orange">
                      {new Date(exam.date).toLocaleDateString('es-UY', { month: 'short' })}
                    </span>
                    <span className="text-xl font-black text-white">
                      {parseInt(exam.date.split("-")[2])}
                    </span>
                  </div>
                  <div>
                    <p className="text-white font-bold">{exam.title.replace('Examen: ', '')}</p>
                    {exam.topic && <p className="text-slate-400 text-xs">{exam.topic}</p>}
                  </div>
                </div>
                <button 
