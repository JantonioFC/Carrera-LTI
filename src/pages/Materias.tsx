    ...
    <>
              ...
              <div className="space-y-2 mt-auto pt-2 border-t border-navy-700/50">
                <div className="flex items-center justify-between">
                  <span className={`px-2 py-0.5 text-xs rounded-full font-medium border ${
                    data[subject.id]?.status === 'aprobada' ? 'bg-green-500/10 text-green-400 border-green-500/20' :
                    data[subject.id]?.status === 'pendiente' ? 'bg-slate-500/10 text-slate-400 border-slate-500/20' :
                    'bg-lti-blue/10 text-lti-blue border-lti-blue/20'
                  }`}>
                    {data[subject.id]?.status === 'aprobada' ? 'Aprobada' : 
                     data[subject.id]?.status === 'pendiente' ? 'Pendiente' : 'En curso'}
                  </span>
                  
                  {data[subject.id]?.status === 'aprobada' && data[subject.id]?.grade !== undefined && (
      ...
      {editingSubject && (
        <EditSubjectModal 
          ...
          currentData={data[editingSubject.id] || { status: editingSubject.status, resources: [] }}
          onSave={(partial) => {
...
function EditSubjectModal({
  subject, currentData, onSave, onClose
}: { 
  subject: Subject; 
  ...
  const [resources, setResources] = useState<SubjectResource[]>(currentData.resources || []);
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in overflow-y-auto pt-10 pb-10">
      <div className="bg-navy-800 rounded-2xl border border-navy-600/50 shadow-2xl w-full max-w-md my-auto flex flex-col max-h-[90vh]">
        <div className="p-5 border-b border-navy-700/50 flex justify-between items-center flex-shrink-0" style={{ borderTop: `4px solid ${subject.color}` }}>
        ...
        <div className="p-5 overflow-y-auto space-y-6 form-scrollbar">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-slate-400 mb-2">Estado de la Materia</label>
              <div className="grid grid-cols-3 gap-2">
                {(['pendiente', 'en_curso', 'aprobada'] as const).map(s => (
                  ...
                  >
                    {s === 'en_curso' ? 'En Curso' : s.charAt(0).toUpperCase() + s.slice(1)}
                  </button>
                ))}
              </div>
            </div>

            <div className={`transition-opacity duration-300 ${status === 'aprobada' ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
              <label className="block text-xs font-medium text-slate-400 mb-1.5">Calificación Final (Opcional)</label>
              <input
                type="number"
                min="1"
                max="12"
          ...
          {/* Resources */}
          <div>
            <label className="block text-xs font-medium text-slate-400 mb-3">Enlaces y Recursos</label>
              ...
              {resources.length === 0 ? (
                ...
                resources.map(res => (
                  <div key={res.id} className="flex items-center gap-2 p-2 rounded-lg bg-navy-900 border border-navy-600/50">
                    <LinkIcon size={14} className="text-lti-blue flex-shrink-0" />
        ...
        <div className="p-5 border-t border-navy-700/50 flex justify-end gap-3 flex-shrink-0">
          ...
          <button 
            onClick={() => onSave({ status, grade: grade ? Number(grade) : undefined, resources })} 
