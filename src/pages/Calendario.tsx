			...
			<div className="bg-navy-800 rounded-2xl border border-navy-600/50 shadow-2xl w-full max-w-md">
				...
				<div className="p-5 border-t border-navy-700/50 flex items-center justify-between">
					...
					<div className="flex gap-3">
						...
						<button
							onClick={() => onSave(form)}
							className="px-4 py-2 text-sm gradient-blue text-white rounded-lg font-medium"
						>
							Guardar
						</button>
					</div>
...
export default function Calendario({
	...
	const getEventsForDay = (day: number) => {
		...
		const events: {
			label: string;
			type: "presencial" | "materia" | "examen";
			color: string;
			event?: PresencialEvent;
		}[] = [];

		// Presenciales
		presenciales.forEach((p) => {
			if (p.date === dateStr)
				events.push({
					label: p.activity,
					type: "presencial",
		...
		// Exams
		if (dateStr >= EXAM_START && dateStr <= EXAM_END)
			events.push({
				label: "Período Exámenes",
				type: "examen",
				color: "#a855f7",
			});
		// Subject starts
		CURRICULUM[0].subjects.forEach((s) => {
			if (s.startDate === dateStr)
				events.push({
					label: `Inicio: ${s.name.slice(0, 20)}...`,
					type: "materia",
					color: s.color,
				});
		});
	...
	const handleSave = (updated: PresencialEvent) => {
		onUpdatePresenciales(
			presenciales.map((p) => (p.id === updated.id ? updated : p)),
		);
							...
							return (
								<div
									...
									className={`min-h-[64px] rounded-lg p-1.5 border transition-colors ${
										isToday
											? "border-lti-blue/50 bg-lti-blue/5"
											: "border-navy-700/30 hover:border-navy-600/50"
									}`}
								>
									<p
										className={`text-xs font-semibold mb-1 ${isToday ? "text-lti-blue" : "text-slate-400"}`}
									>
										{day}
									</p>
							...
							return (
								<div
									...
									className={`p-3 rounded-lg border text-xs space-y-1 ${
										past
											? "border-navy-700/20 opacity-50"
											: "border-navy-700/40 hover:border-lti-orange/30 transition-colors"
									}`}
								>
									<div className="flex items-start justify-between gap-2">
										<p
											className={`font-semibold ${past ? "text-slate-400" : "text-white"}`}
										>
											{formatDate(p.date)}
										</p>
										<button
											onClick={() => setEditingEvent(p)}
											className="text-slate-400 hover:text-lti-blue transition-colors flex-shrink-0"
										>
											<Pencil size={12} />
										</button>
									</div>
