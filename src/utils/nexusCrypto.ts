...
export async function encryptData(plaintext: string, passphrase: string): Promise<string> {
  ...
  const packed = new Uint8Array(salt.length + iv.length + new Uint8Array(cipherBuffer).length);
  packed.set(salt, 0);
...
export async function decryptData(packedBase64: string, passphrase: string): Promise<string> {
