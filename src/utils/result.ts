...
export const success = <T>(data: T): RemoteData<T, never> => ({
	type: "success",
	data,
});
export const failure = <E>(error: E): RemoteData<never, E> => ({
	type: "failure",
	error,
});
...
export const isNotAsked = <T, E>(
	rd: RemoteData<T, E>,
): rd is { type: "not_asked" } => rd.type === "not_asked";
export const isLoading = <T, E>(
	rd: RemoteData<T, E>,
): rd is { type: "loading" } => rd.type === "loading";
export const isSuccess = <T, E>(
	rd: RemoteData<T, E>,
): rd is { type: "success"; data: T } => rd.type === "success";
export const isFailure = <T, E>(
	rd: RemoteData<T, E>,
): rd is { type: "failure"; error: E } => rd.type === "failure";
