    ...
    if (isActive && timeLeft > 0) {
      ...
      if ('Notification' in window && Notification.permission === 'granted') {
        new Notification(mode === 'focus' ? '¡Descanso!' : '¡A estudiar!', {
          body: mode === 'focus' ? 'Tu sesión de estudio terminó. Tómate un descanso.' : 'El descanso terminó. A concentrarse.',
    ...
    <div className="fixed bottom-6 right-6 w-80 bg-navy-800 rounded-2xl border border-navy-600 shadow-2xl z-50 overflow-hidden animate-fade-in">
      <div className="p-4 border-b border-navy-700/50 flex justify-between items-center bg-navy-900/50">
        <div className="flex items-center gap-2">
          {mode === 'focus' ? <BrainCircuit size={16} className="text-lti-blue" /> : <Coffee size={16} className="text-lti-coral" />}
          <h3 className="text-white font-semibold text-sm">Temporizador Pomodoro</h3>
        </div>
        <button onClick={() => setIsOpen(false)} className="text-slate-400 hover:text-white text-lg leading-none cursor-pointer">×</button>
      </div>
