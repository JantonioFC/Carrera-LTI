...
const STEPS: Step[] = [
  {
    ...
    description: "Crea tu proyecto en Google Cloud Console para habilitar las APIs.",
    items: [
    ...
    >
      <div className="p-4 border-b border-navy-800 flex items-center justify-between bg-navy-900/50">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-white font-bold text-sm uppercase tracking-tighter">The Guide: Configuración</h2>
        </div>
        <button onClick={onClose} className="p-1 hover:bg-navy-800 rounded-lg text-slate-400">
          <X size={16} />
        </button>
      </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="space-y-1">
              <p className="text-[10px] text-lti-blue font-bold uppercase tracking-widest">Paso {currentStep + 1} de {STEPS.length}</p>
              <h3 className="text-white font-bold text-lg leading-tight">{STEPS[currentStep].title}</h3>
            </div>
              ...
              {STEPS[currentStep].items.map((item, i) => (
                <div key={i} className="flex gap-3">
                  <div className="w-5 h-5 rounded-full bg-lti-blue/10 border border-lti-blue/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                    <span className="text-lti-blue text-[10px] font-bold">{i + 1}</span>
                  </div>
