...
describe('IPCProtocol.parseIPCMessage', () => {
  ...
  it('should_parse_progress_status', () => {
    const result = parseIPCMessage('{"id":"xyz","status":"progress","data":{"pct":50}}');
  ...
  it('should_throw_IPCValidationError_on_invalid_status_value', () => {
    expect(() => parseIPCMessage('{"id":"abc","status":"unknown"}')).toThrow('IPCValidationError');
  });
  ...
  it('should_throw_IPCValidationError_on_empty_id', () => {
    expect(() => parseIPCMessage('{"id":"","status":"ok"}')).toThrow('IPCValidationError');
  });
});
