...
import { isGrounded, buildGroundedResponse } from '../grounding/GroundingValidator';
...
describe('Cortex E2E — GroundingValidator con índice controlado (REQ-22)', () => {
  ...
  it('should_be_grounded_for_tcp_query', () => {
    const query = 'qué es el three-way handshake';
    const response = 'El three-way handshake consta de SYN, SYN-ACK y ACK para establecer conexión TCP.';
    expect(isGrounded(response, chunks)).toBe(true);
  });

  it('should_be_grounded_for_osi_query', () => {
    const response = 'El modelo OSI tiene 7 capas: física, enlace, red, transporte, sesión, presentación y aplicación.';
    expect(isGrounded(response, chunks)).toBe(true);
  });

  it('should_be_grounded_for_database_query', () => {
    const response = 'La tercera forma normal requiere que no haya dependencias transitivas entre atributos.';
    expect(isGrounded(response, chunks)).toBe(true);
  });
  ...
  it('should_NOT_be_grounded_for_out_of_scope_content', () => {
    const response = 'La fotosíntesis ocurre en los cloroplastos de las células vegetales.';
  ...
  it('should_deduplicate_sources_by_docId', () => {
    const dupChunks: RuVectorChunk[] = [
      { chunkId: 'c1', score: 0.9, content: 'TCP handshake SYN ACK protocolo', docId: 'doc-A' },
      { chunkId: 'c2', score: 0.85, content: 'TCP handshake SYN-ACK establecimiento', docId: 'doc-A' },
    ];
...
describe('Cortex E2E — ConferencePipeline contract', () => {
  ...
  it('should_have_valid_content_in_each_fixture', () => {
    ...
    for (const [file, keyword] of Object.entries(checks)) {
      const content = readFileSync(join(FIXTURES_DIR, file), 'utf8').toLowerCase();
