...
var notifyUserAboutFailure = (message) =>
	new import_obsidian.Notice(`FATAL: ${message}`);
...
function parseStringResponse(result, parsers12, texts, trim = true) {
	asArray(texts).forEach((text) => {
		for (
			let lines = toLinesWithContent(text, trim), i = 0, max = lines.length;
			i < max;
			i++
		) {
			const line = (offset = 0) => {
...
var init_tasks_pending_queue = __esm({
	"src/lib/runners/tasks-pending-queue.ts"() {
		...
		_TasksPendingQueue = class {
			...
			fatal(err) {
				for (const [task, { logger: logger2 }] of Array.from(
					this._queue.entries(),
				)) {
					if (task === err.task) {
				...
				return __async(this, null, function* () {
					...
					try {
						...
						return yield isEmptyTask(task)
							? this.attemptEmptyTask(task, logger2)
							: this.attemptRemoteTask(task, logger2);
					} catch (e) {
...
function splitLine(result, lineStr) {
	...
	function data(index, workingDir, path) {
		...
		if (raw !== "##" && raw !== "!!") {
			result.files.push(
				new FileStatusSummary(path.replace(/\0.+$/, ""), index, workingDir),
			);
		}
	}
}
		...
		parsers7 = [
			...
			new LineParser(
				...
				(result, [major, minor, patch, agent = ""]) => {
					Object.assign(
						result,
						versionResponse(asNumber(major), asNumber(minor), patch, agent),
					);
				},
			),
		];
...
var SimpleGitService = class {
	...
	async gitPush(forcePush = false) {
		...
		logger_default.info(
			`Pushed changes to ${DEFAULT_REMOTE}/${DEFAULT_BRANCH}.`,
		);
	}
...
var GitignoreService = class {
	constructor(basePath, gitService) {
		...
		this.gitignorePath = (0, import_path.resolve)(
			basePath,
			GITIGNORE_FILE_NAME,
		);
...
var UnmergedFilesView = class extends import_obsidian3.Modal {
	...
	onOpen() {
		...
		(_a2 = this.gitService) == null
			? void 0
			: _a2.getConflictingFiles().then((files) => {
					this.contentEl.empty();
					if (files.length > 0) {
						this.contentEl.createEl("h3", {
...
var YaosSettingTab = class extends import_obsidian4.PluginSettingTab {
	...
	addDeviceNameSetting(el) {
		new import_obsidian4.Setting(el)
			.setName("Device name")
			.setDesc(
		...
		if (!this.gitService || !this.gitignoreService) {
			...
			try {
				...
				new import_obsidian5.Notice(
					"Unknown error occurred. Please create an issue.",
				);
